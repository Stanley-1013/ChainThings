...
| 2026-05-07 | cron-schedule-removal | — | Removed `schedule:` block from `.github/workflows/dev-services-cron.yml`; left `workflow_dispatch` only. Kept commented-out `*/15 * * * *` template for easy re-enable when endpoint is on a stable public URL. |
